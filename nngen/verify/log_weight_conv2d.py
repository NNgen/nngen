...
def log_weight_conv2d(input, filter, strides,
    ...
    abs_filter = np.abs(filter)
    pow_filter = 2 ** abs_filter
    log_filter = np.where(filter >= 0, pow_filter, -1 * pow_filter)

    return conv2d(input, log_filter, strides,
                  bias, scale,
