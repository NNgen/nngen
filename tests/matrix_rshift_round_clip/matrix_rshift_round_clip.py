...
def run(a_shape=(15, 15), b_shape=(15, 15),
    ...
    x = ng.rshift_round(a, b, dtype=a_dtype, par=par)
    c = ng.clip(x, dtype=c_dtype, par=par, name='c')

    targ = ng.to_veriloggen([c], 'matrix_rshift_round_clip', silent=silent,
                            config={'maxi_datawidth': axi_datawidth})

    # verification data
    va = ((np.arange(a.length, dtype=np.int64).reshape(a.shape) % [4] + [1]) *
          [2 ** (a_dtype.width // 2)])
    vb = (np.ones(b.length, dtype=np.int64).reshape(b.shape) *
          [a_dtype.width // 4])

    vx = ng.verify.rshift_round(va, vb,
                                dtype=a_dtype,
                                x_dtype=a_dtype, y_dtype=b_dtype)
    vc = ng.verify.clip(vx,
                        dtype=c_dtype,
                        x_dtype=a_dtype)
