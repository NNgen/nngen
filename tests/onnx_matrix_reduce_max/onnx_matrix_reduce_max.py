...
class MatrixReduceMax(nn.Module):
    def __init__(self):
        super(MatrixReduceMax, self).__init__()
...
def run(act_shape=(15, 31),
    ...
    # pytorch model
    model = MatrixReduceMax()

    # Pytorch to ONNX
    onnx_filename = 'onnx_matrix_reduce_max.onnx'
    ...
    for op in operators.values():
        if isinstance(op, ng.reduce_max):
    ...
    targ = ng.to_veriloggen([out], 'onnx_matrix_reduce_max', silent=silent,
