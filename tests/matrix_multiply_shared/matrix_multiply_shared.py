...
def run(a_shape=(15, 15), b_shape=(15, 15), c_shape=(15, 15),
    ...
    x = ng.multiply_shared(a, b, par=par)
    x.output_chainable = False
    x = ng.multiply_shared(x, a, par=par)
    d = ng.add(x, c, dtype=d_dtype, par=par)

    targ = ng.to_veriloggen([d], 'matrix_multiply_add', silent=silent,
