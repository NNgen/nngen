...
def run(act_shape=(1, 7, 7, 15),
    ...
    out = ng.pad(act, padding,
                 dtype=out_dtype, name='out',
                 par=par, value_ram_size=value_ram_size, out_ram_size=out_ram_size)

    targ = ng.to_veriloggen([out], 'matrix_pad', silent=silent,
                            config={'maxi_datawidth': axi_datawidth})

    # verification data
    vact = np.arange(act.length, dtype=np.int64).reshape(act.shape) % [10]

    vout = ng.verify.pad(vact, padding,
                         out_dtype, 'pad', par=par,
                         value_ram_size=value_ram_size,
