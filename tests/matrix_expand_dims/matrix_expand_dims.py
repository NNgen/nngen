...
def run(a_shape=(15, 15), b_shape=(15, 15), axis=0,
    ...
    a = ng.expand_dims(a, axis)
    b = ng.placeholder(b_dtype, shape=b_shape, name='b')
    b = ng.expand_dims(b, axis)
    c = ng.add(a, b, dtype=c_dtype, par=par)

    targ = ng.to_veriloggen([c], 'matrix_expand_dims', silent=silent,
    ...
    ta = ng.verify.expand_dims(va, axis)
    tb = ng.verify.expand_dims(vb, axis)

    vc = ng.verify.add(va, vb,
                       dtype=c_dtype,
