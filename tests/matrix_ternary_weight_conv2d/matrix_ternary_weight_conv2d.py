...
def run(act_shape=(1, 7, 7, 7), weight_shape=(3, 3, 3, 7),
        ...
        act_dtype=ng.int32, weight_dtype=ng.int2,
    ...
    weight = ng.placeholder(weight_dtype, shape=weight_shape, name='weight')
    ...
    out = ng.ternary_weight_conv2d(act, weight, stride,
                                   bias, scale,
                                   rshift_mul, rshift_sum, rshift_out,
                                   act_func, 'SAME',
                                   out_dtype, ng.int32, ng.int32,
                                   'ternary_weight_conv2d',
    ...
    targ = ng.to_veriloggen([out], 'matrix_ternary_weight_conv2d', silent=silent,
    ...
    vweight_ones = np.ones(weight.length, dtype=np.int64).reshape(weight.shape)
    vweight_minusones = np.ones(weight.length, dtype=np.int64).reshape(weight.shape) * [-1]
    vweight_zeros = np.zeros(weight.length, dtype=np.int64).reshape(weight.shape)
    vweight = np.where(vweight > [0], vweight_ones,
                       np.where(vweight < [0], vweight_minusones, vweight_zeros))
    ...
    vout = ng.verify.ternary_weight_conv2d(vact, vweight, stride,
                                           vbias, vscale,
                                           rshift_mul, rshift_sum, rshift_out,
                                           act_func, 'SAME',
                                           out_dtype, ng.int32, ng.int32,
