...
def run(a_shape=(15, 15), b_shape=(15, 15),
    ...
    p = ng.less(x, ng.full_imm((1,), 1, dtype=c_dtype, par=par), par=par)
    q = ng.less(ng.full_imm((1,), 3, dtype=c_dtype, par=par), x, par=par)
    r = ng.logical_or(p, q)
    ...
    if simtype == 'verilator' or lines[-1].startswith('-'):
        rslt = '\n'.join(lines[:-1])
    return rslt
