...
def run(a_shape=(15, 15), b_shape=(15, 15),
    ...
    b_length = functools.reduce(lambda x, y: x * y, b_shape, 1)
    b_value = (np.arange(b_length, dtype=np.int64).reshape(b_shape) + [100]) % [6]
    b = ng.constant(b_value, b_dtype, name='b')
