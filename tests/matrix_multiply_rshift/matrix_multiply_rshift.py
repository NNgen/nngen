...
def run(a_shape=(15, 15), b_shape=(15, 15), c_shape=(15, 15),
    ...
    d = ng.rshift(x, c, dtype=d_dtype, par=par)

    targ = ng.to_veriloggen([d], 'matrix_multiply_rshift', silent=silent,
                            config={'maxi_datawidth': axi_datawidth})

    # verification data
    va = (np.arange(a.length, dtype=np.int64).reshape(a.shape) % [4] + [1]) * [2 ** 9]
    vb = (np.arange(b.length, dtype=np.int64).reshape(b.shape) % [3] + [1]) * [2 ** 9]
    vc = np.arange(c.length, dtype=np.int64).reshape(c.shape) % [1] + [15]

    vx = ng.verify.multiply(va, vb,
                            dtype=ng.int32,
                            x_dtype=a_dtype, y_dtype=b_dtype)
    vd = ng.verify.rshift(vx, vc,
                          dtype=d_dtype,
