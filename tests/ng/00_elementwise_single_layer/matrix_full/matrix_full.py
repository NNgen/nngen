...
def run(a_shape=(15, 15), b_shape=(15, 15),
    ...
    x = ng.add(a, ng.full_imm((1,), 10, dtype=c_dtype, par=par), dtype=c_dtype, par=par)
    y = ng.add(a, ng.full_imm((1,), 20, dtype=c_dtype, par=par), dtype=c_dtype, par=par)
    z0 = ng.add(x, y, dtype=c_dtype, par=par)
    z0.output_chainable = False
    x = ng.add(b, ng.full_imm((1,), 100, dtype=c_dtype, par=par), dtype=c_dtype, par=par)
    y = ng.add(b, ng.full_imm((1,), 200, dtype=c_dtype, par=par), dtype=c_dtype, par=par)
    z1 = ng.add(x, y, dtype=c_dtype, par=par)
    z1.output_chainable = False
    c = ng.add(z0, z1, dtype=c_dtype, par=par)

    targ = ng.to_veriloggen([c], 'matrix_full', silent=silent,
