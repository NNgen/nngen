...
def run(a_shape=(15, 15),
    ...
    b = ng.reduce_min(a, axis, keep_dims, par=par)

    targ = ng.to_veriloggen([b], 'matrix_reduce_min', silent=silent,
                            config={'maxi_datawidth': axi_datawidth})

    # verification data
    va = np.arange(a.length, dtype=np.int64).reshape(a.shape) % [16] - [9]
