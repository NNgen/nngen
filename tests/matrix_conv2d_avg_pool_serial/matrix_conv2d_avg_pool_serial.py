...
def run(act_shape=(1, 7, 7, 15), weight_shape=(7, 3, 3, 15),
    ...
    out = ng.avg_pool_serial(tmp, ksize=ksize,
                             strides=pool_stride,
                             sum_dtype=ng.int32, dtype=out_dtype, par=par)

    targ = ng.to_veriloggen([out], 'matrix_conv2d_avg_pool_serial', silent=silent,
    ...
    vtmp = ng.verify.conv2d(vact, vweight, conv2d_stride,
                            vbias, vscale,
                            rshift_mul, rshift_sum, rshift_out,
                            act_func, 'SAME',
                            out_dtype, ng.int32, ng.int32,
    ...
    vout = ng.verify.avg_pool_serial(vtmp, ksize,
                                     pool_stride, 'SAME',
